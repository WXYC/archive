...
- Auth context: `useAuth()` hook provides `isAuthenticated`, `login`, `sendLoginCode`, `verifyLoginCode`, `completeSignIn`, `logout`, `getToken`. DJ-level access is checked via `isDJRole()` from `@wxyc/shared/auth-client`.
